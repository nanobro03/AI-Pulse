...
# # ------------------ CONFIG ------------------
# st.set_page_config(page_title="Cyberjaya Incidents Dashboard", layout="wide")


# # ------------------ IMAGE DISPLAY LOGIC (FIXED FOR OLDER STREAMLIT) ------------------
# # This logic runs first. If it finds "?view_photo=" in the URL, it displays the image
# # in a container at the top and stops the rest of the app from loading to focus the view.
...
#             st.image(image, caption=f"Photo for Incident ID: {incident_id_to_show}", use_column_width=True)
...
#     # Add a "Close" link that reloads the page to its base URL, removing the photo view.
#     # We use markdown for maximum compatibility with all Streamlit versions.
#     st.markdown("### [← Click here to close and return to the dashboard](/)", unsafe_allow_html=True)

# # This 'else' block contains your entire original dashboard.
# # It will only run if the 'view_photo' parameter is NOT in the URL.
# else:
#     st.title("🚧 Cyberjaya Road Incident Dashboard")
